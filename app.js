...
async function calculateDelivery(address){const seq=++deliverySeq;showCalculating();try{const payload={action:"calculateDelivery",full_address:address};const r=await apiPost(payload);if(seq!==deliverySeq)return;const dist=Number(r.distance_km),price=Number(r.patient_price);estimatedDistance=dist;livePatientPrice=Number.isFinite(price)&&price>0?price:null;roadDistanceVerified=Number.isFinite(dist);$("summaryAddress").textContent=address||"Lokasi perangkat";renderFare();const z=$("zoneHint");z.hidden=false;z.textContent=dist>15?`Jarak ${dist.toFixed(2)} km — memerlukan konfirmasi petugas.`:`Jarak ${dist.toFixed(2)} km • Tarif ${rupiah(livePatientPrice)}`}
