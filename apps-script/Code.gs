...
function createOrder_(p){['orderId','name','whatsapp','doctor','address','area','paymentMethod'].forEach(k=>{if(!String(p[k]||'').trim())throw Error('Data wajib belum lengkap: '+k);});if(p.consent!==true)throw Error('Persetujuan data wajib.');const km=Number(p.distanceKm);if(!Number.isFinite(km)||km<=0)throw Error('Jarak jalan belum tervalidasi.');const c=cfg_();let base=km<=c.REGULAR_RADIUS_KM?c.REGULAR_CHARGE:(km<=c.AUTO_RADIUS_MAX_KM?c.REGULAR_CHARGE+Math.ceil(km-c.REGULAR_RADIUS_KM)*c.EXTRA_KM_RATE:'');const manual=km>c.AUTO_RADIUS_MAX_KM;let discount=0,voucherCode='';if(!manual&&p.voucherCode){voucherCode=String(p.voucherCode).trim();try{discount=voucherDiscount_(voucherCode,Number(base));}catch(e){throw Error('Voucher: '+e.message);}}const finalCharge=manual?'':Number(base)-discount;const orders=sheet_(SHEETS.ORDERS,ORDER_HEADERS);if(find_(orders,1,p.orderId))return {ok:true,duplicate:true,orderId:p.orderId};const patient=upsertPatient_(p),address=upsertAddress_(p,patient.id);orders.appendRow([p.orderId,new Date(),patient.id,safe_(p.name),phone_(p.whatsapp),safe_(p.doctor),address.id,safe_(p.address),safe_(p.area),Number(p.lat),Number(p.lng),km,safe_(p.pricingZone),safe_(p.operationalZone),manual?'':finalCharge,manual?'Ya':'Tidak','',manual?c.OUTER_ZONE_SERVICE_FEE:0,safe_(p.paymentMethod),'Pending','Waiting','Pending','','',safe_(voucherCode),discount,manual?'':base]);return {ok:true,orderId:p.orderId,patientId:patient.id,addressId:address.id,finalFare:finalCharge,discountAmount:discount};}
